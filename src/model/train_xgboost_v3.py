...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 4. Greenup rate (growth velocity from baseline to peak)
    for idx in INDICES:
        baseline_col = f"{idx}_mean_baseline"
        if baseline_col in df.columns and f"{idx}_peak_value" in df.columns:
            peak_stage = df[f"{idx}_peak_stage"].copy()
            # Avoid division by zero; stage indices range 0-5, so add 1 to denominator
            peak_stage = peak_stage.replace(0, np.nan).add(1, fill_value=np.nan)
            df[f"{idx}_greenup_rate"] = (
                (df[f"{idx}_peak_value"] - df[baseline_col]) / peak_stage
            )

    # 5. Senescence rate (decline velocity from peak to maturity)
    for idx in INDICES:
        maturity_col = f"{idx}_mean_maturity"
        if maturity_col in df.columns and f"{idx}_peak_value" in df.columns:
            peak_stage = df[f"{idx}_peak_stage"].copy()
            stages_after = (len(STAGES) - 1 - peak_stage).replace(0, np.nan)
            df[f"{idx}_senescence_rate"] = (
                (df[f"{idx}_peak_value"] - df[maturity_col]) / stages_after.add(1, fill_value=np.nan)
            )
    ...
    # 12. SAR-based features (radar backscatter dynamics)
    for idx in SAR_INDICES:
        # Peak and range for SAR indices
        stage_cols = [f"{idx}_mean_{s}" for s in STAGES if f"{idx}_mean_{s}" in df.columns]
        if stage_cols:
            df[f"{idx}_peak_stage"] = df[stage_cols].idxmax(axis=1)
            df[f"{idx}_peak_stage"] = df[f"{idx}_peak_stage"].map(
        ...
        # Within-field heterogeneity for SAR
        spreads = []
        for stage in STAGES:
            p10 = f"{idx}_p10_{stage}"
            p90 = f"{idx}_p90_{stage}"
    ...
    # 13. SAR + Optical fusion features (complementary information)
    for stage in ["vegetative", "flowering", "grain_fill"]:
        rvi_col = f"RVI_mean_{stage}"
        ndvi_col = f"NDVI_mean_{stage}"
        if rvi_col in df.columns and ndvi_col in df.columns:
            denom = df[ndvi_col].replace(0, np.nan)
            df[f"RVI_NDVI_ratio_{stage}"] = df[rvi_col] / denom

        cr_col = f"CR_mean_{stage}"
        evi_col = f"EVI_mean_{stage}"
        if cr_col in df.columns and evi_col in df.columns:
            df[f"CR_EVI_interaction_{stage}"] = df[cr_col] * df[evi_col]

    # 14. Cumulative SAR indices (proxy for total backscatter)
    for idx in SAR_INDICES:
        mean_cols = [f"{idx}_mean_{s}" for s in STAGES if f"{idx}_mean_{s}" in df.columns]
        if mean_cols:
            df[f"{idx}_cumulative"] = df[mean_cols].sum(axis=1)
...
def add_null_indicators(X: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in high_null_cols:
        X[f"{col}_is_null"] = X[col].isnull().astype(np.float32)
...
if __name__ == "__main__":
    ...
    X, y, feature_names, le = load_data(db_path, min_stages=3)

    model = train_and_evaluate(
        X, y, feature_names, le,
        n_folds=5,
        optuna_trials_xgb=80,
        optuna_trials_et=40,
