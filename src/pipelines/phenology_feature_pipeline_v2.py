...
# Optical: Statistical API (server-side stats with cloud masking)
...
# SAR: Statistical API — Sentinel-1 GRD (IW, dual-pol VV+VH)
# VV/VH converted to dB; CR = cross-pol ratio (VH/VV linear);
...
class PhenologyFeaturePipeline:
    ...
    def _process_stage(self, coords, stage_name, base_date, stage_windows,
        ...
        sar_result = None
        if self.fetch_sar:
            sar_result = self._fetch_sar_stats(coords, ds, de)
    ...
    # ── SAR Backfill ─────────────────────────────────────────────────────
    # Iterates existing rows that have NULL SAR columns, finds their KML,
    # fetches only Sentinel-1 data, and UPDATEs in place.
    # Optical data is NOT re-requested.
    ...
    def backfill_sar(self, kml_root: str):
        ...
        rows = conn.execute(
            ...
            f'FROM phenology_features WHERE "{first_sar_col}" IS NULL'
        ).fetchall()

        if not rows:
            logger.info("No rows need SAR backfill — all populated")
            conn.close()
            return
        ...
        for i, (rowid, field_id, crop_label, planting_date_str) in enumerate(rows, 1):
            ...
            for stage_name in STAGES:
                day_start, day_end = stage_windows[stage_name]
                window_start = planting_date + timedelta(days=day_start)
                window_end = planting_date + timedelta(days=day_end)
                ds = window_start.strftime("%Y-%m-%d")
                de = window_end.strftime("%Y-%m-%d")
                ...
                if result is not None:
                    ...
                    logger.info("    %s: SAR OK (%.0f%%)", stage_name, sar_pct * 100)
                else:
                    for idx in SAR_INDICES:
                        for stat in STATS:
                            sar_features[f"{idx}_{stat}_{stage_name}"] = np.nan
                    logger.info("    %s: SAR no data", stage_name)
        ...
        conn.close()
        logger.info("SAR backfill complete — %d updated, %d skipped", updated, skipped)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Phenology Feature Pipeline v2 (optical + SAR)")
    parser.add_argument("--mode", choices=["full", "backfill-sar"], default="full",
                        help="'full' = process new KMLs (optical+SAR), "
                             "'backfill-sar' = add SAR to existing rows")
