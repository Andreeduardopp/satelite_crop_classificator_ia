...
def expand_sar_window(day_start: int, day_end: int,
                      min_days: int = MIN_SAR_WINDOW_DAYS) -> tuple[int, int]:
    """Expand a stage window symmetrically so SAR has >= min_days of coverage."""
...
class PhenologyFeaturePipeline:
    ...
    def _process_stage(self, coords, stage_name, base_date, stage_windows,
        ...
        # -- Optical (uses original crop-specific window) -----------------
        optical_result = None
        ...
        # -- SAR (uses expanded window to guarantee S1 coverage) ----------
        sar_result = None
        if self.fetch_sar:
            sar_day_start, sar_day_end = expand_sar_window(day_start, day_end)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Phenology Feature Pipeline v3 (expanded SAR windows)")
