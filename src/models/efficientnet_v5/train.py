...
Uso:
    python src/models/efficientnet_v5/train.py
"""
...
TABELA           = 'culturas'
IMG_SIZE         = (224, 224)
BATCH_SIZE       = 128          # aumentado para GPU — mais throughput por step
BATCH_SIZE_EMBED = 256          # embedding extraction sem gradientes → batch grande
EPOCHS_FASE1     = 15
EPOCHS_FASE2     = 20
LR_FASE1         = 1e-4
LR_FASE2         = 5e-5
...
FINE_TUNE_BLOCOS = 3
NUM_WORKERS      = 4            # threads de I/O do DataLoader (ajuste se tiver mais cores)

DEVICE    = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
USE_AMP   = DEVICE.type == 'cuda'   # Automatic Mixed Precision (FP16) só na GPU

# Habilita autotuner do cuDNN — encontra o algoritmo de conv mais rápido
# para os shapes fixos de entrada (224×224)
if DEVICE.type == 'cuda':
...
def carregar_dados(
    ...
    for row in rows:
        ...
        for p in paths:
            # Paths no DB sao relativos a src/ (ex: './processadas/...')
...
def treinar_fase(
    ...
    for epoch in range(epochs):
        ...
        for images, lab in loader_treino:
            # non_blocking=True sobrepõe a transferência H→D com o compute da CPU
            images = images.to(DEVICE, non_blocking=True)
            lab    = lab.to(DEVICE, non_blocking=True)

            # zero_grad(set_to_none=True) é mais rápido que zero_grad()
            optimizer.zero_grad(set_to_none=True)

            # Automatic Mixed Precision — forward em FP16, loss scale automático
...
def treinar_xgboost(
    ...
    # tree_method='hist' + device='cuda' usa a GPU para construir as árvores
...
def main() -> None:
    ...
    # persistent_workers=True mantém os workers vivos entre epochs (evita overhead de fork)
    ...
    # GradScaler para AMP — escala o loss para evitar underflow em FP16
    ...
    treinar_fase(modelo, loader_tr, loader_val, opt, criterion,
                 EPOCHS_FASE1, cw_tensor, "Fase1", patience=5, scaler=scaler)
    ...
    treinar_fase(modelo, loader_tr, loader_val, opt, criterion,
                 EPOCHS_FASE2, cw_tensor, "Fase2", patience=5, scaler=scaler)
    ...
    try:
        ...
        metrics_dict = {
            ...
            "f1_per_class": {c: float(f) for c, f in zip(CLASSES, f1_per)},
            "tempo_medio_ms": float(ms_per),
        }
        ...
        logging.warning(f"Erro ao salvar métricas: {e}")
