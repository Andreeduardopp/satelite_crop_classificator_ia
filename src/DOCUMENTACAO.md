...
```
KML do talhão → Sentinel Hub → response.png   (bruta)
                              ↓
                       cria_mascara()  → mascaras.png   (silhueta)
                              ↓
                       aplica_mascara() → processadas.png  (entrada do modelo)
```

### 1.1 Entrada: KML do talhão

Cada talhão é descrito por um arquivo KML contendo o polígono do campo agrícola.
gerador_dataframe.py varre uma pasta de KMLs e produz
o dataframe extraindo, do nome do arquivo, os metadados:
...
Para cada talhão, o pipeline solicitauma imagem por DAP (dias após plantio), onde os DAPs são definidos
por cultura:
...
Porque é importante o mapeamento desses dias? 
Para criar um modelo robusto de IA para categorização, uma única imagem de satélite não basta, pois muitas plantas apresentam uma cobertura verde genérica e indistinguível em fotografias isoladas. A captura de imagens nestes dias específicos após o plantio serve para mapear a assinatura fenológica (o ciclo de vida biológico) de cada cultura. Ao alimentar o seu modelo espaço-temporal com estes intervalos precisos, a IA não analisa apenas a aparência da planta, mas sim as derivadas matemáticas da sua curva de crescimento, avaliando a taxa de mudança dos índices de vegetação (como o NDVI) ao longo do tempo.

Cada conjunto de datas isola os momentos críticos que diferenciam a biologia de cada espécie. Para culturas de verão (soja, milho e feijão), as datas registam a emergência, a velocidade de fechamento do dossel e o pico de floração; para os cereais de inverno (trigo e aveia), capturam a diferença sutil no ritmo de maturação de ambos. O arroz é perfeitamente identificado pela transição drástica do reflexo da água inundada (dia 15) para a folhagem densa, enquanto o café, sendo uma cultura perene, exige uma janela mais ampla e espaçada para que o modelo detecte as flutuações lentas e contínuas de umidade e biomassa durante o desenvolvimento dos grãos.

Para cada DAP, chamamos o Sentinel Hub com:
...
A resposta é gravada em ordena as candidatas e retorna apenas a melhor (menor cobertura de nuvens)
...
### 1.6 O que chega ao modelo

Para cada talhão presente no banco, lêmos a coluna `imagens_processadas`, descartamos os
...
### 2.1 Carregamento bruto — 

Recebemos do dataset criado três listas paralelas:
...
Talhões sem nenhuma imagem válida são silenciosamente excluídos. 
...
Cada talhão produz uma tupla com 5 tensores de tamanho fixo, mesmo quando o talhão tem menos de
3 imagens:
...
| `label` | `()` | `long` | Índice da classe em `CLASSES` |
...
desbalanceado (no caso atual, soja e milho dominam). Com `replacement=True`, talhões
de classes raras são reamostrados várias vezes por época; o modelo vê variações geométricas diferentes a cada visita.
...
A classe EfficientNetTemporalé composta por quatro blocos:
