...
EfficientNetB0 com sequência multi-temporal (TensorFlow/Keras) — versão v4.

Arquitetura (single-stream RGB):
    1. Augment (treino) → RGB por-timestep → EfficientNetB0 (compartilhado)
       → GAP → 1280-dim
    2. FiLM(dia): tokens = features * (1 + γ) + β   (γ, β zero-init)
    3. MultiHeadAttention × 2 → mean pool
       → Dense → classificação

Convenção de caminhos:
    RGB  : ./processadas/mascara_{uuid}_v_d{dia}.png

Fases:
    1. Base RGB congelada — treina FiLM + temporal + cabeça
    2. Fine-tuning — descongela as últimas 20 camadas do EfficientNet

Uso:
    python src/treinamento/treinar_classificador_v4.py
"""
...
log_filename = os.path.join(
    LOG_DIR, f'treino_efficientnet_v4_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.txt'
)
...
LR_FASE2        = 1e-5
LABEL_SMOOTHING = 0.1
MODELO_SAIDA    = './modelos/classificador_cultura_efficientnet_v4'
...
def carregar_dados(db_path: str) -> tuple[list[list[tuple[str, int]]], list[int]]:
    ...
    (caminho_png, dia) ORDENADOS por dia, pertencentes ao mesmo talhão.
    """
    classe_para_id = {c: i for i, c in enumerate(CLASSES)}
    registros: list[list[tuple[str, int]]] = []
    labels: list[int] = []

    descartados_sem_png = 0
    ...
    logging.info(
        ...
        f"descartados s/ PNG: {descartados_sem_png}"
    )
...
def carregar_imagem_rgb(caminho: str) -> np.ndarray:
    """RGB float32 em [0, 255], redimensionada — preprocess acontece no modelo."""
...
def criar_dataset(registros: list[list[tuple[str, int]]], labels: list[int],
    ...
    """
    Cada sample é um talhão com até MAX_SEQ_LEN timesteps.

    Saída:
        ((rgb, dias, mask), label_onehot)
        rgb   : (T, H, W, 3)  float32 em [0, 255]
        dias  : (T,)          float32 normalizados
        mask  : (T,)          float32 binário (1 = timestep real)
        label : (C,)          one-hot
    """
    ...
    def gerador():
        for items, label in zip(registros, labels):
            ...
            rgb = np.zeros((MAX_SEQ_LEN, IMG_SIZE[0], IMG_SIZE[1], 3), dtype=np.float32)
            dias = np.zeros(MAX_SEQ_LEN, dtype=np.float32)
            mask = np.zeros(MAX_SEQ_LEN, dtype=np.float32)

            for i in range(seq_len):
                caminho_png, dia = items[i]
                rgb[i] = carregar_imagem_rgb(caminho_png)
                dias[i] = dia / MAX_DIA
                mask[i] = 1.0
            ...
            yield (rgb, dias, mask), label_onehot
...
class EfficientNetTemporalModelV4(tf.keras.Model):
    """
    Single-stream RGB + FiLM + Temporal Attention.
    """

    def __init__(self, num_classes: int, **kwargs):
        super().__init__(**kwargs)

        self.augment_rgb = tf.keras.Sequential(
            [
        ...
        self.backbone_rgb = tf.keras.applications.EfficientNetB0(
            include_top=False,
            weights='imagenet',
            input_shape=(IMG_SIZE[0], IMG_SIZE[1], 3),
        )
        self.backbone_rgb.trainable = False
        self.pool_rgb = tf.keras.layers.GlobalAveragePooling2D()

        # FiLM
        self.dia_hidden = tf.keras.layers.Dense(64, activation='relu')
        self.dia_gamma = tf.keras.layers.Dense(
            FEATURE_DIM, kernel_initializer='zeros', bias_initializer='zeros'
        )
        self.dia_beta = tf.keras.layers.Dense(
            FEATURE_DIM, kernel_initializer='zeros', bias_initializer='zeros'
        )
    ...
    def call(self, inputs, training=False):
        ...
        # ── RGB features ──────────────────────────────────────────────
        rgb_flat = tf.reshape(rgb, (-1, IMG_SIZE[0], IMG_SIZE[1], 3))
        rgb_flat = self.augment_rgb(rgb_flat, training=training)
        rgb_flat = tf.keras.applications.efficientnet.preprocess_input(rgb_flat)
        feat_flat = self.backbone_rgb(rgb_flat, training=training)
        feat_flat = self.pool_rgb(feat_flat)                     # (B*T, 1280)
        tokens_raw = tf.reshape(feat_flat, (B, T, FEATURE_DIM))

        # ── FiLM: modulação condicional pelo dia ──────────────────────
        dia_hidden = self.dia_hidden(tf.expand_dims(dias, -1))  # (B, T, 64)
        gamma = self.dia_gamma(dia_hidden)                       # (B, T, 1280)
        beta = self.dia_beta(dia_hidden)                         # (B, T, 1280)
        tokens = tokens_raw * (1.0 + gamma) + beta
    ...
    def descongelar_ultimas_camadas(self, n_camadas: int = 20):
        """Descongela as últimas N camadas do EfficientNet (Fase 2)."""
        self.backbone_rgb.trainable = True
        for layer in self.backbone_rgb.layers[:-n_camadas]:
...
def main() -> None:
    ...
    logging.info(
        "Modelo EfficientNetB0 V4: RGB + FiLM + Augmentation + LabelSmoothing"
    )
    ...
    # 5. Modelo
    logging.info(f"Criando EfficientNetB0 V4 — RGB {FEATURE_DIM}-dim tokens")
    modelo = EfficientNetTemporalModelV4(len(CLASSES))
    ...
    # ── Fase 1: Base RGB congelada ─────────────────────────────────────
    logging.info("=== Fase 1: Treinando FiLM + temporal + cabeça (backbone RGB congelado) ===")
    ...
    tempo_medio = (t_total / n_samples) * 1000 if n_samples else 0.0
