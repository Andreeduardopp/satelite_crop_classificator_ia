...
EfficientNetB0 com SEQUÊNCIA multi-temporal (TensorFlow/Keras) — versão v3.

Diferenças em relação ao v2:
    1. Dataset maior (6000 talhões por cultura, em `sample_treino_6k.db`)
    2. Fusão temporal via FiLM (Feature-wise Linear Modulation):
       em vez de somar o embedding do dia às features visuais, o dia é
       usado para gerar γ e β que MODULAM as features:
           tokens = features * (1 + γ(dia)) + β(dia)
       Isso força o modelo a usar a informação temporal em vez de ignorá-la.
    3. Data augmentation agressivo (flip, rotation, zoom, contrast, brightness)
       aplicado por-timestep APENAS durante o treino.
    4. Label smoothing 0.1 (CategoricalCrossentropy com labels one-hot).

Arquitetura:
    1. Augment (treino) → cada imagem → EfficientNetB0 (compartilhado)
       → GlobalAvgPool → 1280-dim
    2. Dia normalizado → Dense → γ (1280-dim) e β (1280-dim)
       tokens = features * (1 + γ) + β
    3. Sequência → MultiHeadAttention (2 camadas) → mean pool
       → Dense → classificação

Fases:
    1. Base congelada — treina cabeça + FiLM + temporal attention
    2. Fine-tuning  — descongela as últimas 20 camadas do EfficientNet

Uso:
    python src/treinamento/treinar_classificador_v3.py
"""
...
log_filename = os.path.join(
    LOG_DIR, f'treino_efficientnet_v3_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.txt'
)
...
LR_FASE2        = 1e-5
LABEL_SMOOTHING = 0.1
MODELO_SAIDA    = './modelos/classificador_cultura_efficientnet_v3'
...
def carregar_imagem_raw(caminho: str) -> np.ndarray:
    """
    Carrega imagem como float32 no range [0, 255] em RGB, redimensionada.
    A normalização do EfficientNet acontece DENTRO do modelo.
    """
...
def criar_dataset(registros: list[list[tuple[str, int]]], labels: list[int],
    ...
    imagens temporais + dias normalizados + mask + label one-hot.

    As imagens são entregues em [0, 255] RGB; o preprocess do EfficientNet
    (e o augmentation) acontecem dentro do modelo.
    """
    ...
    def gerador():
        for items, label in zip(registros, labels):
            ...
            yield (images, dias, mask), label_onehot
...
class EfficientNetTemporalModelV3(tf.keras.Model):
    """
    EfficientNetB0 + FiLM + Temporal Attention para classificação multi-temporal.

    Mudanças vs v2:
      - Augmentation por-timestep (só no treino)
      - Fusão temporal via FiLM (γ, β) em vez de soma
      - Preprocess do EfficientNet aplicado internamente (após augmentation)
    """
    ...
    def call(self, inputs, training=False):
        ...
        # Achatar timesteps para o backbone: (B*T, H, W, 3)
        images_flat = tf.reshape(images, (-1, IMG_SIZE[0], IMG_SIZE[1], 3))

        # Augmentation (só treino) nos pixels [0, 255]
        images_flat = self.augment(images_flat, training=training)

        # Preprocess do EfficientNet (normalização dependente do backbone)
        ...
        # Modulação: features * (1 + γ) + β
        # γ e β inicializam em zero → no primeiro step é equivalente a `features`
...
def main() -> None:
    ...
    logging.info("Modelo EfficientNetB0 V3: 6k/classe + FiLM + Augmentation + LabelSmoothing")
    ...
    # 5. Modelo
    logging.info("Criando EfficientNetB0 V3 + FiLM + Temporal Attention...")
    modelo = EfficientNetTemporalModelV3(len(CLASSES))
    ...
    logging.info("=== Fase 1: Treinando cabeça + FiLM + temporal attention (base congelada) ===")
